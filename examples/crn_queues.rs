...
impl ArrivalEvent {
    fn schedule(sim_state: &mut Store, events: &mut EventQueue<Store, F64Time>) {
        ...
        events.schedule(Self {}, F64Time(arrival_time))
...
impl ServiceEvent {
    fn schedule(customer: Customer, store: &mut Store, events: &mut EventQueue<Store, F64Time>) {
        ...
        events.schedule(Self {}, F64Time(service_time))
...
impl OkEvent<Store, F64Time> for ServiceEvent {
    fn execute(&mut self, simulation_state: &mut Store, event_queue: &mut EventQueue<Store, F64Time>) {
        ...
        if simulation_state.customer_queue.is_empty() {
            ...
            let next_customer = simulation_state.customer_queue.pop_front()
...
impl EndEvent {
    fn schedule(time: F64Time, events: &mut EventQueue<Store, F64Time>) {
        events.schedule(Self {}, time)
            .expect("end time should be positive");
    }
}
...
fn main() {
    ...
    println!("M/M/1 with rate 6.0 served {} customers with {} total time in queue", mm1_results.0, mm1_results.1);
    println!("M/M/2 with rate 3.0 served {} customers with {} total time in queue", mm2_results.0, mm2_results.1);
    println!("M/M/3 with rate 2.0 served {} customers with {} total time in queue", mm3_results.0, mm3_results.1);
}
